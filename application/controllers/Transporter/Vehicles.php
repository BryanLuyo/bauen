...
class Vehicles extends MY_Controller{
	...
	public function add(){
		...
		$rules=array(
			...
			array(
				...
				'errors'=>array(
					'unique_plate_no'=>'La placa %s ya esta en uso.'
				)
			),
			array(
				'field'=>'plate_trailer',
		...
		if($this->form_validation->run()===true){
			...
			if($vehicle_id>0){
				$this->session->set_flashdata('success','Los detalles del vehículo han sido guardados.');
				redirect(BASE_FOLDER_TRANS.'vehicles');
			}
			else{
				$this->session->set_flashdata('error','No se pudo guardar los detalles del vehículo.');
			}
		}
		// assing data 
		$data['trailers']=$this->gettrailers();
	...
	public function edit($vehicle_id=0){
		...
		if($vehicle_id>0){
			...
			if(empty($vehicle)){
				$this->session->set_flashdata('error','No se encontraron los detalles del vehículo.');
				redirect(BASE_FOLDER_TRANS.'vehicles');
			}
			$data['vehicle']=$vehicle;
		}
		else{
			$this->session->set_flashdata('error','No se encontró la información del vehículo.');
			redirect(BASE_FOLDER_TRANS.'vehicles');
		}
		$this->load->library(array('form_validation'));
		$rules=array(
		...
		if($this->form_validation->run()===true){
			...
			$this->session->set_flashdata('success','Los detalles del vehiculo han sido actualizados.');
			redirect(BASE_FOLDER_TRANS.'vehicles');
		}
		// assing data 
		$data['trailers']=$this->gettrailers();
		...
		if($this->form_validation->run()===true){
			if(isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
				...
				if(!empty($file_name)){
					...
					if($document_id>0){
						$this->session->set_flashdata('success','El archivo se ha subido correctamente.');
						redirect(BASE_FOLDER_TRANS.'vehicles/adddocument/'.$vehicle_id);
					}
					else{
						$this->session->set_flashdata('success','No se pudo subir el archivo.');
					}
				}
				else{
					$this->session->set_flashdata('error','No se pudo subir el archivo.');
				}
			}
			else{
