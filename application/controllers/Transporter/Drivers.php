...
class Drivers extends My_Controller{
	...
	public function add(){
		...
		if($this->form_validation->run()===true){
			...
			if($driver_id>0){
				$this->session->set_flashdata('success','Los detalles del conductor han sido guardados.');
				redirect(BASE_FOLDER_TRANS.'drivers');
			}
			else{
				$this->session->set_flashdata('error','No se pudo guardar los detalles del conductor.');
			}
		}
	...
	public function edit($driver_id=0){
		...
		if($driver_id>0){
			...
			if(empty($driver)){
				$this->session->set_flashdata('error','No se encontraron los detalles del conductor.');
				redirect(BASE_FOLDER_TRANS.'drivers');
			}
			$data['driver']=$driver;
		}
		else{
			$this->session->set_flsahdata('error','No se encontró la información del conductor.');
			redirect(BASE_FOLDER_TRANS.'drivers');
		}
		...
		$rules=array(
			...
			array(
				...
				'errors'=>array(
					'valid_phone_no'=>'El campo %s debe tener un número telefónico válido.',
					'user_unique_phone_no'=>'El correo %s ya esta en uso.'
				)
			),
			array(
				'field'=>'password',
		...
		if($this->form_validation->run()===true){
			...
			$this->session->set_flashdata('success','Los detalles del conductor han sido actualizados.');
			redirect(BASE_FOLDER_TRANS.'drivers');
		}
	...
	public function documents($driver_id=0){
		...
		if($driver_id>0){
			...
			$this->session->set_flashdata('error','No se encontró la información del conductor.');
			redirect(BASE_FOLDER_TRANS.'drivers');
		}
		// find doct list 
		$documenttype_id=0;
		...
		if($this->form_validation->run()===true){
			...
			if(isset($_FILES['image']['name']) && !empty($_FILES['image']['name'])){
				...
				if(!empty($file_name)){
					...
					if($document_id>0){
						$this->session->set_flashdata('success','El documento ha sido subido correctamente.');
						redirect(BASE_FOLDER_TRANS.'drivers/adddocument/'.$driver_id);
					}
					else{
						$this->session->set_flashdata('success','No se pudo subir el documento.');
					}
				}
				else{
					$this->session->set_flashdata('error','No se pudo subir el documento.');
				}
			}
			else{
				$this->session->set_flashdata('error','El documento es requerido.');
			}
		}
		// assing data 
		$doc_cond=array(
	...
	public function viewdocument($document_id=0){
		...
		if($document_id>0){
			...
			if(empty($document)){
				$this->session->set_flashdata('error','No se encontraron los detalles del documento.');
				redirect(BASE_FOLDER_TRANS.'drivers');
			}
			...
			$this->session->set_flashdata('error','No se encontró la información del documento.');
			redirect(BASE_FOLDER_TRANS.'drivers');
		}
